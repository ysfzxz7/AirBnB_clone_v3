...
* [HTTP access control (CORS)](https://intranet.alxswe.com/rltoken/WfKwKtaROCybta0_E849AQ)
...
![concept image](./readme_images/concept.png)
...
But once again, let’s work on a new [codebase](https://github.com/alexaorrico/AirBnB_clone_v2.git).
...
![task 1 image](./readme_images/task_1.jpg)
...
Update `DBStorage` and `FileStorage`, adding two new methods. <b>All changes should be done in the branch `storage_get_count`</b>:
...
For this task, you <b>must</b> make a pull request on GitHub.com, and ask at least one of your peer to review and merge it.
...
<i>(No need to have a pretty rendered output, it’s a JSON, only the structure is important)</i>
...
* Update the method `to_dict()` of `BaseModel` to remove the `password` key <b>except when it’s used by `FileStorage` to save data to disk</b>. Tips: default parameters
...
[-> Abdelemjid Essaid](https://github.com/abdelemjidessaid/AirBnB_clone_v3/tree/master/api)